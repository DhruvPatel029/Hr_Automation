								...
								<div class="col-12">
									...
									<%@taglib prefix="f" uri="http://java.sun.com/jstl/core_rt"%>
										<table id="order-listing" class="table">
											...
											<tbody>
											<f:forEach items="${loanList}" var="i" varStatus="j">
												<tr>
													<td>${j.count}</td>
													<td>${i.designationVO.designationName}</td>
													<td>${i.loanLimit}</td>
													<td>${i.timeLimitYears}</td>
												    <td>${i.rateOfinterest}</td>
													
													<td><a href="editLoan?id=${i.id}"><i class="mdi mdi-pencil "></i></a>
													<a href="deleteLoan?id=${i.id}"><i class="mdi mdi-delete lead text-danger ml-1 "></i></a></td>
												</tr>
												</f:forEach>
											</tbody>
										</table>
